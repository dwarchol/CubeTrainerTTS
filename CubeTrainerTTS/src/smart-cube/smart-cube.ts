...
import * as def from './smart-cube-definitions';
import { GanGen2CubeEncrypter, GanGen3CubeEncrypter, GanGen4CubeEncrypter, MoyuCubeEncrypter } from './smart-cube-encrypter';
import {
...
async function connectGanCube(customMacAddressProvider?: MacAddressProvider): Promise<GanCubeConnection> {
    // console.log("Requesting GAN Smart Cube device...");
    ...
    var device: BluetoothDeviceWithMAC = await navigator.bluetooth.requestDevice(
        {
            ...
            optionalServices: [def.GAN_GEN2_SERVICE, def.GAN_GEN3_SERVICE, def.GAN_GEN4_SERVICE, def.MOYU_SERVICE],
    ...
    for (let service of services) {
        ...
        if (serviceUUID == def.GAN_GEN2_SERVICE) {
            ...
            conn = await GanCubeClassicConnection.create(device, commandCharacteristic, stateCharacteristic, encrypter, driver);
            break;
        } else if (serviceUUID == def.MOYU_SERVICE) {
            console.log("Detected Moyu cube service");
            let commandCharacteristic = await service.getCharacteristic(def.MOYU_COMMAND_CHARACTERISTIC);
            let stateCharacteristic = await service.getCharacteristic(def.MOYU_STATE_CHARACTERISTIC);
            let key = def.GAN_ENCRYPTION_KEYS[2];
            let encrypter = new MoyuCubeEncrypter(new Uint8Array(key.key), new Uint8Array(key.iv), salt);
