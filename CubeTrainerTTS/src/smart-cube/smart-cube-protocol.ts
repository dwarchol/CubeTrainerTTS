...
import { now, toKociembaFacelets } from 'gan-web-bluetooth'; //units
import { SmartCubeEncrypter } from './smart-cube-encrypter';
...
class GanCubeClassicConnection implements GanCubeConnection, GanCubeRawConnection {
    ...
    async sendCommandMessage(message: Uint8Array): Promise<void> {
        ...
        var uEncryptedMessage = new Uint8Array(encryptedMessage);
        return this.commandCharacteristic.writeValue(uEncryptedMessage);
    }

    onStateUpdate = async (evt: Event) => {
        console.log("State update event received", evt);
