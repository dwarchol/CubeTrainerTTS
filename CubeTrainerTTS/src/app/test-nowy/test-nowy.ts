...
@Component({
  selector: 'app-test-nowy',
  imports: [],
  templateUrl: './test-nowy.html',
  styleUrl: './test-nowy.css',
})
class TestNowy {
  constructor(private cubeService: CubeService) { }

  async connect() {
    this.cubeService.connectMOYUCube();
    await connect(this.cubeService);
  }
  ct() {
    // console.log("Wysyłanie danych do kostki...");
  }
}
